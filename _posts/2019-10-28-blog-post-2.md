...
In this post, I will review Spatial Statistics class projects. The main models includes stationary Gaussian Processes Bayesian inference, Predictive Gaussian Process, and finally Gaussian Markov Random Field. The goal is to highlight model properties, implementation details, and present some real data analysis with these three models applied to the Albedo satellite data in North America. 


## Data 

Albedo is the proportion of light or radiation reflected by surface. We are interested in Albedo over land, since its variation over the ocean is small. To control project scope, the training data was obtained by sampling 500 locations each within the the block with lon $[-126, -108]$ and lat $[34, 62]$ among 0.19 million points for the two Satellites GOES 075, GOES 135. Locations within the ocean is intentionally removed. Albedo is a measure of light reflectivity which takes values in $[0, 1]$. 

![Raw and sampled data from two Satellites](http://jesscyzhao.github.io/images/raw_data.png)
...
Let $x_1^*, \cdots, x_s^*$ be a set of knots. 
...
$$
$$
cov(f(x_i), f^*) = [\rho(x_i, x_1^*|\theta), \cdots, \rho(x_i, x_s^*|\theta)]
$$
where $\rho$ is the covariance function defined through parameter $\theta$. 

$$
Var(f^*)_{i, j} = (\rho(x_i^, x_j^*|\theta)) = C^{*-1}
$$

Using the conditional distribution of MVN, we have
$$
y_i = \mu + cov(f(x_i), f^*)^T Var(f^*)^{-1}f^* + \epsilon_i
$$

In matrix form this is, 
$$
y = \mu + [c(x_1), \cdots c(x_n)]C^{*-1}f^* + \epsilon
$$
...
We define $K$ matrix in the kernel convolution by $K = [c(x_1)'; \cdots; c(x_s)']C^{*-1}$. Where $c(x_1) = [\rho(x_1, x_1^*), \cdots, \rho(x_1, x_s^*)]$.  
...
$$
y_i = \mu + cov(f(x_i), f^*)var(f^*)^{-1}f^* + \epsilon
$$

$$
cov(f(x_i), f^*)var(f^*)^{-1}f^* = E[f(x_i)|f^*]
$$
...
$$
y_i = \mu + E[f(x_i)|f^*] + \tilde{\epsilon_i}
$$
...
$$
Var(E[f(x_i)|f^*]) + \tilde{\sigma^2}
$$

The difference in variability of regression function = $var(f(x_i))- var(E[f(x_i)|f^*]) = E[Var(f(x_i)|f^*)]$
...
The spatial effect is modeled through a process convolution of spherical Bezier kernel and a GMRF prior on the coefficients. The transformed data $Y_1(s^{(1)})$ is a vector of $n_1$ observations at locations $s^{(1)}$ from the first data source, i.e GOES 135; $Y_2(s^{(2)})$ is the vector of $n_2$ observations at locations $s^{(2)}$ from the second data source, i.e. GOES 075. $X_1$ and $X_2$ are $n_1 \times k_1$, $n_2 \times k_2$ covariates matrices for GOES 135 and GOES 075 respectively. Let $\boldsymbol{\beta_1}$ and $\boldsymbol{\beta_2}$ be $k_1 \times 1$, $k_2 \times 1$ coefficient vectors for GOES 135 and GOES 075. The model is formulated in the following way:
...
where $n_i$ is the number of neighbours knots $i$ has. $i \sim j$ means knot $i$ and knot $j$ are adjacent to each other in the first order, i.e. j is either directly above, below, or next to i from left or right. On a regular grid, the inner knots always have 4 adjacent neighbours; the edge knots have 3 neighbours and the corner knots have only 2.
...
$$
$$
Z_\delta|- \sim MVN_p(V_\delta K_\delta^{(2)'}(Y_2 - X_2 \beta_2 - K_\mu^{(2)}Z_\mu), V_\delta)
$$

5. $\lambda_\mu|-$, $\lambda_\delta|-$
$$
...
The actual sample sequence is 3, 4, 6, 4, 5, 1, 2. The initial values for $Z_\mu, Z_\delta$ are vectors of 0, the initial values for $\beta_1, \beta_2$ are OLS estimates of $Y_1$ and $Y_2$ on $X_1$ and $X_2$ respectively.
...
![Kriging result for Satellite A using Predictive GP](http://jesscyzhao.github.io/images/kriging_075_25-1.png)

![Kriging result for Satellite B using Predictive GP](http://jesscyzhao.github.io/images/kriging_135-1.png)

![Kriging result for Satellite A using GMRF](http://jesscyzhao.github.io/images/075_krig_m3.png)

![Kriging result for Satellite B using GMRF](http://jesscyzhao.github.io/images/135_krig_m3.png)

![Inference for latent mean and difference processes using GMRF](http://jesscyzhao.github.io/images/latent_surface.png)
