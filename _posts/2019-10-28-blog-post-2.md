...
$$
$$
cov(f(x_i), f^*) = [\rho(x_i, x_1^*\mid\theta), \cdots, \rho(x_i, x_s^*\mid\theta)]
$$
where $\rho$ is the covariance function defined through parameter $\theta$. 

$$
Var(f^*)_{i, j} = (\rho(x_i^, x_j^*\mid\theta)) = C^{*-1}
$$
...
$$
cov(f(x_i), f^*)var(f^*)^{-1}f^* = E[f(x_i)\mid f^*]
$$
...
$$
y_i = \mu + E[f(x_i)\mid f^*] + \tilde{\epsilon_i}
$$
...
$$
Var(E[f(x_i)\mid f^*]) + \tilde{\sigma^2}
$$
...
1. Kriging result for Satellite A using Predictive GP

![](http://jesscyzhao.github.io/images/kriging_075_25-1.png)

2. Kriging result for Satellite B using Predictive GP
![](http://jesscyzhao.github.io/images/kriging_135-1.png)

3. Kriging result for Satellite A using GMRF

![](http://jesscyzhao.github.io/images/075_krig_m3.png)

4. Kriging result for Satellite B using GMRF
![](http://jesscyzhao.github.io/images/135_krig_m3.png)

5. Inference for latent mean and difference processes using GMRF
