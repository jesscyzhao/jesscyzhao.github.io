...
In this post will be a review of the Spatial Statistics class projects. The main topics includes stationary Gaussian Processes Bayesian inference, Predictive Gaussian Process, and finally Gaussian Markov Random Field. The goal is to highlight model properties, implementation details, and present some real data analysis with these three models applied to the Albedo satellite data in North America. 


<!-- ## Data 

To control project scope, the training data was obtained by sampling 500 locations within the the block with lon $[-126, -108]$ and lat $[34, 62]$ among 0.19 million points where the Albedo measure comes from two Sattelight, GOES 075, GOES 135. Locations within the ocean is intentionally removed. Albedo is a measure of light reflectivity which takes values in $[0, 1]$. 
