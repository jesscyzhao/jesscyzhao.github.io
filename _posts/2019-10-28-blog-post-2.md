...
We define $K$ matrix in the kernel convolution by $K = [c(x_1)^\prime; \cdots; c(x_s)^\prime]C^{*-1}$. Where $c(x_1) = [\rho(x_1, x_1^{*}), \cdots, \rho(x_1, x_s^{*})]$.  
...
The difference in variability of regression function = $var(f(x_i))- var(E[f(x_i)\mid f^*]) = E[Var(f(x_i)\mid f^*)]$
...
5. $\lambda_\mu\mi-$, $\lambda_\delta \mid -$
$$
